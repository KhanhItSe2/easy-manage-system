...
<body>
    ...
    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST" class="form" id="form">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" placeholder="Enter ethnic ID" name="ethnicId" required>
        </div>
        <div class="form-group">
            <label>Ethnic Name:</label>
            <input type="text" class="form-control" placeholder="Enter ethnic name" name="ethnicName" required>
        </div>
    ...
    <div class="container-fluid" id="align-table">
        ...
        $count = mysqli_query($conn, "SELECT COUNT(ethnicId) as total FROM `ethnic`");
        $data = mysqli_fetch_assoc($count);
        ?>
        <div class="menu">
            <h2>Ethnic List (<?php echo $data['total'] ?>)</h2>
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td><?= $row['day']; ?></td>
                        <td><a href="./delete.php?id=<?= $row['ethnicId'] ?>"><button type="button" class="btn btn-danger" name="delete">Delete</button></a></td>
                    <tr>
                    <?php } ?>
            </tbody>
        </table>
