...
<body>
    ...
    <div class="container-fluid" id="align-table">
        ...
        $count = mysqli_query($conn, "SELECT COUNT(acaID) as total FROM `academic`");
        $data = mysqli_fetch_assoc($count);
        ?>
        <div class="menu">
            <h2>Academic List (<?php echo $data['total'] ?>)</h2>
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td><?= $row['day']; ?></td>
                        <td><a href="./delete.php?id=<?= $row['acaId'] ?>"><button type="button" class="btn btn-danger" name="delete">Delete</button></a></td>
                    <tr>
                    <?php } ?>
            </tbody>
        </table>
