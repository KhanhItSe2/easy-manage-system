...
<body>
    ...
    <div class="container-fluid" id="align-table">
        <?php
        if (!empty($_SESSION['is_login_employer'])) {
            ...
            $count = mysqli_query($conn, "SELECT COUNT(recruitmentId) as total FROM `recruitment`");
            ...
            if (!empty($where)) {
                $sql = "SELECT  * FROM `recruitment`  as r 
              JOIN `ethnic` as e ON r.ethnic = e.ethnicId
              ...
              where (" . $where . ") LIMIT " . $item_per_page . " OFFSET " . $offset;
                ...
                JOIN `position` as p ON p.posId = r.position
                JOIN `branch` as b ON b.branchId = r.branch
                ORDER BY `recruitmentId` LIMIT " . $item_per_page . " OFFSET " . $offset;
            }
            $result = mysqli_query($conn, $sql);
        }

        ?>
        <form id="" action="recruitment.php?action=search" method="POST">
            <div class="menu">
                <h2>Recruitment List (<?php echo $data['total'] ?>)</h2>
                <div class="float-item">
                    <label>ID: </label>
                    <input type="text" name="recruitmentId" value="">
                    <label>Name: </label>
                    <input type="text" name="fullName" value="">
                    <button type="submit" class="btn btn-danger"><span class="glyphicon glyphicon-filter"></span> Search</button>
                    ...
                    <button type="button" class="btn btn-info" onclick="openNav()"><span class="glyphicon glyphicon-pencil"></span> Recruit</button>
                </div>
            </div>
        </form>
                ...
                ?>
                    <tr>
                        ...
                        <?php if ($row['status'] == 'Processing') { ?>
                            <td><button type="button" class="btn btn-warning"><?= $row['status']; ?></button></td>
                        ...
                        <td><a href="./profile.php?recruitment_id=<?= $row['recruitmentId'] ?>&recruitmentId_name=<?= $row['fullName'] ?>"><button type="button" class="btn btn-primary" name="detail">Detail</button></a>
