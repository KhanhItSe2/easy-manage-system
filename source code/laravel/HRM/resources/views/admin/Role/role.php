...
<body>
    ...
    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST" class="form" id="form">
        ...
        <div class="form-group">
            <label>Role ID:</label>
            <input type="text" class="form-control" placeholder="Enter role ID" name="roleId" required>
        </div>
        <div class="form-group">
    ...
    <div class="container-fluid" id="align-table">
        ...
        $count = mysqli_query($conn, "SELECT COUNT(roleId) as total FROM `role`");
        $data = mysqli_fetch_assoc($count);
        ?>
        <div class="menu">
            <h2>Role List (<?php echo $data['total'] ?>)</h2>
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td><?= $row['day']; ?></td>
                        <td><a href="./delete.php?id=<?= $row['roleId'] ?>"><button type="button" class="btn btn-danger" name="delete">Delete</button></a></td>
                    <tr>
                    <?php } ?>
            </tbody>
        </table>
