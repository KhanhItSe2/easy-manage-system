...
<body>
    ...
    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST" class="form" id="form">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" placeholder="Enter religion ID" name="religionId" required>
        </div>
        <div class="form-group">
            <label>Religion Name:</label>
            <input type="text" class="form-control" placeholder="Enter religion name" name="religionName" required>
        </div>
    ...
    <div class="container-fluid" id="align-table">
        ...
        $count = mysqli_query($conn, "SELECT COUNT(religionId) as total FROM `religion`");
        $data = mysqli_fetch_assoc($count);
        ?>
        <div class="menu">
            <h2>Religion List (<?php echo $data['total'] ?>)</h2>
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td><?= $row['day']; ?></td>
                        <td><a href="./delete.php?id=<?= $row['religionId'] ?>"><button type="button" class="btn btn-danger" name="delete">Delete</button></a></td>
                    <tr>
                    <?php } ?>
            </tbody>
        </table>
