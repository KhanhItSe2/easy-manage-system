...
<body>
    ...
    <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST" class="form" id="form">
        ...
        <div class="form-group">
            <label>ID:</label>
            <input type="text" class="form-control" placeholder="Enter position ID" name="posId" required>
        </div>
        <div class="form-group">
            <label>Position:</label>
            <input type="text" class="form-control" placeholder="Enter position name" name="posName" required>
        </div>
    ...
    <div class="container-fluid" id="align-table">
        ...
        $count = mysqli_query($conn, "SELECT COUNT(posID) as total FROM `position`");
        $data = mysqli_fetch_assoc($count);
        ?>
        <div class="menu">
            <h2>Position List (<?php echo $data['total'] ?>)</h2>
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td><?= $row['day']; ?></td>
                        <td><a href="./delete.php?id=<?= $row['posId'] ?>"><button type="button" class="btn btn-danger" name="delete">Delete</button></a></td>
                    <tr>
                    <?php } ?>
            </tbody>
        </table>
