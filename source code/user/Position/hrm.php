...
<body>
    ...
    <div class="container-fluid" id="align-table">
        ...
        if (!empty($_SESSION['is_login_manager'])) {
            ...
            $posId = $_GET['id'];
            $count = mysqli_query($conn, "SELECT COUNT(empId) as total FROM `employee` WHERE posId = '$posId'");
            ...
            if (!empty($where)) {
                $sql = "SELECT  * FROM `employee` WHERE posId = '$posId' where (" . $where . ") LIMIT " . $item_per_page . " OFFSET " . $offset;
            } else {
                $sql = "SELECT  * FROM `employee` WHERE posId = '$posId' ORDER BY `empId` LIMIT " . $item_per_page . " OFFSET " . $offset;
            }
            $result = mysqli_query($conn, $sql);
        }
        ?>
        <form id="" action="hrm.php?action=search&id=<?= $posId ?>" method="POST">
            <div class="menu">
