...
<body>
    ...
    <div class="container-fluid" id="align-table">
        ...
        if (!empty($_SESSION['is_login_manager'])) {
            ...
            $Id = $_GET['id'];
            $count = mysqli_query($conn, "SELECT COUNT(empId) as total FROM `employee` WHERE deptId = '$Id'");
            ...
            if (!empty($where)) {
                $sql = "SELECT  * FROM `employee` WHERE deptId = '$Id' where (" . $where . ") LIMIT " . $item_per_page . " OFFSET " . $offset;
            } else {
                $sql = "SELECT  * FROM `employee` WHERE deptId = '$Id' ORDER BY `empId` LIMIT " . $item_per_page . " OFFSET " . $offset;
            }
